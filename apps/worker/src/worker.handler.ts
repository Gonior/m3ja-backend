...
export class WorkerHandler {
  constructor(
    ...
    this.logger.setContext(WorkerHandler.name);
  }
  @EventPattern(EVENT.WORKER_UPLOAD_AVATAR)
  async handleUpload(@Payload() data: IUploadEvent) {
    this.logger.debug(`receive avatar data from API with user id ${data.userId}`);
    const userId = data.userId;
    let newData = (await this.resizeImageService.resizeImage(data)) as IUploadEvent;
    newData.userId = userId;
    this.logger.debug(`send avatar data to API with user id ${newData.userId}`);
