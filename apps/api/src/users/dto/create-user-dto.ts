import { ERROR_MESSAGE_INVALID_EMAIL, MAX_LENGTH_DISPLAY_NAME, MAX_LENGTH_EMAIL, MIN_LENGTH_DISPLAY_NAME, MIN_LENGTH_PASSWORD } from "@app/shared";
...
export class CreateUserDto implements Omit<TUser, 'id' | 'createdAt' | 'updatedAt'> {
  ...
  @MinLength(MIN_LENGTH_DISPLAY_NAME, {message : minError("display name", MIN_LENGTH_DISPLAY_NAME)})
  @MaxLength(MAX_LENGTH_DISPLAY_NAME, {message : maxError('display name', MAX_LENGTH_DISPLAY_NAME)})
